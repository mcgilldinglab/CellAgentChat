...
import ot
from output_paths import artifact_dir, artifact_path
...
class ExpDataset(Dataset):
    def __init__(self, x, y, device=None):
        # Keep dataset tensors on CPU so DataLoader pin_memory can stage
        # batches efficiently before the training loop moves them to CUDA.
        self.x = torch.as_tensor(x, dtype=torch.float32).cpu()
        self.y = torch.as_tensor(y, dtype=torch.float32).cpu()
...
def train(adata, lig_uni, rec_uni, tf_uni, rec_tf_uni, lr_pairs, path="model.pt", epochs=50, lr=0.1, batch=256, device=None):
    ...
    dataloader = DataLoader(dataset=dataset, **_loader_kwargs(device, shuffle=False, batch_size=batch))
...
def perform_iteration(model, mat, C, criterion, batch=256, device=None, return_predictions=True):
    ...
    y_pred = torch.cat(predictions, dim=0) if return_predictions else None
...
def feature_selection(model, mat, C, rec_uni, start=0, perc=100, n_shuffles=10, batch=256, seed=None, device=None):
    print("Performing feature selection to obtain conversion rates...")
    model, device = _move_model_to_device(model, device=device)
    criterion = nn.MSELoss()
    _, cost = perform_iteration(
        model, mat, C, criterion, batch=batch, device=device, return_predictions=False
    )
...
def get_target_genes(
    ...
    baseline_predictions, _ = perform_iteration(model, mat, C, nn.MSELoss(), batch=batch, device=device)
    ...
    for rec in tqdm(receptors, desc="Getting target genes", unit="receptor"):
        ...
        for _ in range(n_shuffles):
            ...
            shuffled_prediction, _ = perform_iteration(
                model, perturbed_matrix, C, nn.MSELoss(), batch=batch, device=device
            )

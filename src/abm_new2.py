...
from model_setup_new import ExpDataset, block_receptors
...
class CellModel(Model):
    ...
    def __init__(
        ...
        self._coords = self._get_coordinate_array()
        ...
        # Create agents
        agent_iter = tqdm(range(self.num_agents), desc="Initializing agents", unit="agent", leave=False)
        for i in agent_iter:
            ...
            if self._coords is not None:
                self._agent_distance_coords[a] = np.asarray(self._coords[i], dtype=float)

            # Coordinates
            if self.dist_on and self._coords is not None:
                grid_x, grid_y = self._project_to_grid(self._coords[i])
    ...
    def _get_coordinate_array(self):
        raw_2d = ["x_raw", "y_raw"]
        raw_3d = ["x_raw", "y_raw", "z_raw"]
        true_2d = ["x_true", "y_true"]
        true_3d = ["x_true", "y_true", "z_true"]
        obs_2d = ["x", "y"]
        obs_3d = ["x", "y", "z"]

        for columns in (raw_3d, raw_2d, true_3d, true_2d, obs_3d, obs_2d):
    ...
    def get_normalized_dist(self):
        ...
        if self._coords is None:
            raise ValueError("Distance mode requires x/y or x/y/z coordinates in adata.obs")

        coords = self._coords[cell_ids]
    ...
    def calc_normalized_dist(self):
        if self._coords is None:
            raise ValueError("Distance mode requires x/y or x/y/z coordinates in adata.obs")
        distances = _torch_pairwise_distances(self._coords, device=self._distance_device)
